...
- M3 → **INVERTIDO pelo Rick ("recuperar o mindroot") e RESOLVIDO ✅**: descoberto que o CD automático do projeto Vercel `mindroot-v2` SOBREVIVEU à transferência do repo (rsmramalho/mindroot-v2 → Atom-HS/atom, redirect manteve o webhook) — cada push no master deployava sozinho este tempo todo, com env correto. Verificado com login real + /hoje servindo. **Deploy oficial: `mindroot-v2.vercel.app` · domínio `www.mindroot.com.au` FUNCIONA.** O redundante era o meu deploy estático `atom-deploy` — projeto removido do Vercel (404 confirmado). Pendência residual: o projeto Vercel vive na conta antiga (migrar quando consolidar contas) + nota de identidade: domínio diz "mindroot", produto diz "Atom" — decidir domínio definitivo antes do release (lei v1.3: nome público = Atom).
